...
std::vector<float> encoder_impl::generate_freq_deviation(const std::vector<int>& symbols) {
    const int sample_rate = 48000;
    const float baud_rate = 6.25f;
    const float freq_shift = 6.25f;

    int samples_per_symbol = sample_rate / static_cast<int>(baud_rate);
    std::vector<float> freq_dev_vals(symbols.size() * samples_per_symbol, 0.0f);

    for (size_t i = 0; i < symbols.size(); ++i) {
        float freq_deviation = symbols[i] * freq_shift;

        for (int j = 0; j < samples_per_symbol; ++j) {
            freq_dev_vals[i * samples_per_symbol + j] = freq_deviation;
        }
    }
    d_logger->info("FSK signal generated");
    return freq_dev_vals;
}
...
void encoder_impl::generate_waveform() {
    try {
        ...
        std::vector<float> freq_dev_vals = generate_freq_deviation(symbols);
        d_logger->info(" FSK size: {}", freq_dev_vals.size());

        d_waveform = freq_dev_vals;
