...
std::vector<float> encoder_impl::s_gaussian_coefs;
bool encoder_impl::s_coefs_initialized = false;

encoder::sptr encoder::make(std::string message) {
    return gnuradio::make_block_sptr<encoder_impl>(message);
}

encoder_impl::encoder_impl(std::string message_text)
    : gr::sync_block(
          "encoder", gr::io_signature::make(0, 0, 0),
          gr::io_signature::make(1 /* min outputs */, 1 /*max outputs */, sizeof(output_type))),
      d_message_obj(message_text), d_sample_idx(0), d_waveform_generated(false) {
...
void encoder_impl::init_gaussian_coefs() {
    if (s_coefs_initialized)
        return;

    s_gaussian_coefs = gr::filter::firdes::gaussian(1.0f,     // gain
                                                    48000.0f, // sampling rate
                                                    6.25f,    // symbol rate (baud rate)
                                                    2.0f      // BT = 2.0 for FT8
    );

    d_fir_filter = gr::filter::fir_filter_fff::make(1, s_gaussian_coefs);

    s_coefs_initialized = true;
    d_logger->info("Gaussian filter coefficients initialized");
}

std::vector<float> encoder_impl::generate_rectangular_fsk(const std::vector<int>& symbols) {
...
std::vector<float> encoder_impl::apply_fir_filter(const std::vector<float>& fsk_signal) {
    if (!d_fir_filter) {
        d_logger->error("FIR filter not initialized");
        return fsk_signal;
    }

    auto tb = gr::make_top_block("fir_filter_temp");
    auto source = gr::blocks::vector_source_f::make(fsk_signal, false);
    auto sink = gr::blocks::vector_sink_f::make();

    tb->connect(source, 0, d_fir_filter, 0);
    tb->connect(d_fir_filter, 0, sink, 0);

    tb->run();

    std::vector<float> filtered_data = sink->data();
    d_logger->info("Fir filter applied");
...
void encoder_impl::generate_waveform() {
    try {
        ...
        d_waveform = apply_fir_filter(rectangular_fsk);
        d_logger->info("Final waveform size: {}", d_waveform.size());
...
int encoder_impl::work(int noutput_items, gr_vector_const_void_star& input_items,
