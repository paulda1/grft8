...
BOOST_AUTO_TEST_CASE(test_message_prod_basic)
{
    ...
    pmt::pmt_t test_message = pmt::string_to_symbol("CQ DX VE4ABC EN19");
    ...
    const float freq_shift = 6.25f; //*todo: double check
    ...
    const float vco_sensitivity = (2.0f * M_PI)/ sample_rate;
    ...
    auto symbol_to_freq = gr::blocks::multiply_const_ff::make(freq_shift);
    auto add_base_freq = gr::blocks::add_const_ff::make(base_freq);
    auto vco = gr::blocks::vco_f::make(sample_rate, vco_sensitivity, 1.0f);
    ...
    //     "./ft8_signal.wav",                    // filename
    //     1,                                     // n_channels (mono)
    //     static_cast<unsigned int>(sample_rate), // sample_rate (must be unsigned int)
    //     gr::blocks::FORMAT_WAV,                // format (WAV file)
    //     gr::blocks::FORMAT_PCM_16,             // subformat (16-bit PCM)
