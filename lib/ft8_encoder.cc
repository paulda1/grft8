...
std::bitset<77>
ft8_encoder::encode_euvhfx(const message &message)
{
...
std::bitset<77>
ft8_encoder::encode_free_text(const message &message)
{
    const auto& parsed_data = message.get_parsed_data();
    std::bitset<71> f71;
    uint8_t n3 = 0;
    ...
    for (int i = 0; i < 71; ++i) { //can't use pack bit because71 bits is larger than 64 bit integer
...
std::bitset<77>
ft8_encoder::encode_nonstd_call(const message &message)
{
    const auto& parsed_data = message.get_parsed_data();
    uint16_t h12 = 0;
    uint64_t c58 = 0;
...
std::bitset<77>
ft8_encoder::encode_telemetry(const message &message)
{
    const auto& parsed_data = message.get_parsed_data();
    uint8_t i3 = 0; 
...
std::bitset<77>
ft8_encoder::encode_rtty_ru(const message &message)
{
    const auto& parsed_data = message.get_parsed_data();
    bool t1 = 0;
...
std::bitset<77>
ft8_encoder::encode_field_day (const message &message)
{
  const auto& parsed_data = message.get_parsed_data();
  uint32_t c28a = 0, c28b = 0;
  bool R1 = 0;
...
std::bitset<77>
ft8_encoder::encode_dexpedition (const message &message)
{
  const auto& parsed_data = message.get_parsed_data();
  uint32_t c28a = 0, c28b = 0;
  uint16_t h10 = 0;
...
uint16_t
ft8_encoder::encode_euvhf_serial(const message::ParsedData& parsed_data)
{
...
uint32_t
ft8_encoder::g6_to_25(const message::ParsedData& parsed_data)
{
...
uint8_t
ft8_encoder::encode_contest_report(const message::ParsedData& parsed_data)
{
...
uint16_t
ft8_encoder::encode_contest_info(const message::ParsedData& parsed_data)
{
    if (parsed_data.contest_info.empty()) {
        return 0;
    }
...
uint32_t
ft8_encoder::encode_28 (const message::ParsedData& parsed_data, size_t& callsign_idx)
{
      ...
      if (std::regex_match(modifier, std::regex(R"([A-Z]{1,4})")))
        {
          ...
          switch(modifier.length())
            ...
            case 3:
              letter_val = (modifier[0] - 'A') * 26 * 26
                           + (modifier[1] - 'A') * 26 + (modifier[2] - 'A');
              return 1760 + letter_val;
            case 4:
              letter_val = (modifier[0] - 'A') * 26 * 26 * 26
                           + (modifier[1] - 'A') * 26 * 26
                           + (modifier[2] - 'A') * 26 + (modifier[3] - 'A');
...
uint32_t
ft8_encoder::std_call_to_28 (const std::string &callsign)
{
  ...
  if (i1 == std::string_view::npos || i2 == std::string_view::npos
    ...
    {
      d_logger.error (
          "Invalid character trying to be encoded in standard message");
    }
...
ft8_encoder::string_to_hash(const std::string &string)
{
...
uint8_t
ft8_encoder::encode_arrl_section(const message::ParsedData& parsed_data)
{
...
uint64_t
ft8_encoder::nonstd_to_58(const message::ParsedData& parsed_data)
{
...
std::bitset<71>
ft8_encoder::free_text_to_f71 (const message::ParsedData& parsed_data)
{
  constexpr std::string_view a = " 0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ+-./?";
...
uint16_t
ft8_encoder::g4_to_15 (const message::ParsedData& parsed_data)
{
  if (!parsed_data.grid_squares.empty())
    {
      const std::string& grid = parsed_data.grid_squares[0];
      return (grid[0] - 'A') * 18 * 10 * 10
             + (grid[1] - 'A') * 10 * 10
             + (grid[2] - '0') * 10 + (grid[3] - '0');
    }
  return 0;
}

uint8_t
ft8_encoder::encode_fdclass (const message::ParsedData& parsed_data)
{
...
uint8_t
ft8_encoder::encode_r2 (const message::ParsedData& parsed_data)
{
  if (parsed_data.has_rrr) return 1;
  if (parsed_data.has_rr73) return 2;
  if (parsed_data.has_73) return 3;
  return 0;
}

uint8_t
ft8_encoder::encode_sigreport (const message::ParsedData& parsed_data)
{
