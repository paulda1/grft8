...
uint32_t ft8_encoder::encode_28(std::string& temp_msg, const message& message) {
    if (temp_msg.find("DE ") != std::string::npos) {
        temp_msg = std::regex_replace(temp_msg, std::regex("\\bDE\\b\\s*"), "");
        ;
        return 0;
    }
    if (temp_msg.find("QRZ ") != std::string::npos) {
        temp_msg = std::regex_replace(temp_msg, std::regex("\\bQRZ\\b\\s*"), "");
        return 1;
    }

    std::regex cq_only(R"(^CQ\s*$)");
    if (std::regex_match(temp_msg, cq_only)) {
        temp_msg = std::regex_replace(temp_msg, std::regex("\\bCQ\\b\\s*"), "");
        return 2;
    }
    ...
    if (std::regex_search(temp_msg, two_letter, cq_two_letter)) {
        ...
        uint32_t letter_val = (letters[0] - 'A') * 26 + (letters[1] - 'A'); // essnetially base 26
    ...
    if (std::regex_search(temp_msg, three_letter, cq_three_letter)) {
        ...
        uint32_t letter_val =
            (letters[0] - 'A') * 26 * 26 + (letters[1] - 'A') * 26 + (letters[2] - 'A');
    ...
    if (std::regex_search(temp_msg, four_letter, cq_four_letter)) {
        ...
        uint32_t letter_val = (letters[0] - 'A') * 26 * 26 * 26 + (letters[1] - 'A') * 26 * 26 +
...
uint32_t ft8_encoder::std_call_to_28(std::string& msg) {
    ...
    if (i1 == std::string_view::npos || i3 == std::string_view::npos ||
        ...
        d_logger.error("Invalid character tyring to be encoded in standard message");
    }
...
uint64_t ft8_encoder::nonstd_to_58(std::string& msg) {
...
std::bitset<71> ft8_encoder::free_text_to_f71(std::string& msg) {
    constexpr std::string_view a = " 0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ+-./?";
    std::string temp_msg = msg;
    temp_msg.resize(13, ' ');
...
uint16_t ft8_encoder::g4_to_15(std::string& temp_msg,
                               const message& message) { // in original protocol RRR, RR73, and 73
                                                         // are also encoded here, removed
    ...
    while (keywords >> keyword) {
        if (message.is_grid_square(keyword)) {
            uint16_t encoded_15 = (keyword[0] - 'A') * 18 * 10 * 10 + (keyword[1] - 'A') * 10 * 10 +
...
uint32_t ft8_encoder::g6_to_15(const message& message) {
    std::string temp_msg = message.get_message();
    std::istringstream keywords(temp_msg);
    std::string keyword;
    ...
    while (keywords >> keyword) {
        if (message.is_grid_6square(keyword)) {
            uint32_t encoded_25 = (keyword[0] - 'A') * 18 * 10 * 10 * 24 * 24 +
                                  ...
                                  (keyword[2] - '0') * 10 * 24 * 24 + (keyword[3] - '0') * 24 * 24 +
...
bool ft8_encoder::encode_fdclass(char fd_class) {
...
bool ft8_encoder::encode_p1(std::string& msg) {
...
bool ft8_encoder::encode_r1(std::string& msg) {
...
bool ft8_encoder::encode_t1(std::string& msg) {
...
uint8_t ft8_encoder::encode_r2(std::string& msg) {
    if (msg == "RRR")
        return 1;
    if (msg == "RR73")
        return 2;
...
uint8_t ft8_encoder::encode_sigreport(std::string& msg) {
    int db = std::stoi(msg);
    return (db + 30) / 2;
}
...
//     int tot_samples = (symbols.size() + 4) * samples_per_symbol; //4 extra bymbol periods of
//     padding std::vector<float> waveform(tot_samples, 0.0f);
